...
namespace LibraryApp
{
    ...
    {
        string sessionValue;
        public Tags(string sessionValue)
        {
            InitializeComponent();
            ...
            MySqlCommand command = new MySqlCommand("SELECT * FROM `tags` ", db.getConnection());
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand command = new MySqlCommand("DELETE FROM `tags` WHERE `tag` = @item",db.getConnection());
            command.Parameters.Add("@item", MySqlDbType.VarChar).Value = textBox1.Text;
            db.openConnection();
            adapter.InsertCommand = command;
            if (command.ExecuteNonQuery() == 1)
            {
                errorText.Text = "Тэг удалён успешно";
            ...
            MySqlCommand command1 = new MySqlCommand("SELECT * FROM `tags` ", db.getConnection());
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            DB db = new DB();
            MySqlDataAdapter adapter = new MySqlDataAdapter();
            MySqlCommand command = new MySqlCommand("UPDATE `tags` SET `idTag`=@item, `tag`=@item2 WHERE `tag`=@item1",db.getConnection());
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            DB db = new DB();
            MySqlDataAdapter adapter = new MySqlDataAdapter();
            MySqlCommand command = new MySqlCommand("INSERT INTO `tags` VALUES (NULL, @item)", db.getConnection());
